...
type ProductUpdateRpcIn struct {
	ID           int64   `json:"id"`
...
type ProductUpdateRpcOut struct {
	ID *int64
}

type ProductUpdateError struct {
	Error string
}
...
func (u *handle) Update(w http.ResponseWriter, r *http.Request) {
	...
	CreateListInput, err := validateProductUpdate(bodyBytes)
	if err != nil {
...
func validateProductUpdate(bodyBytes []byte) (*product2.ProductUpdateInput, error) {
	listInt := product2.ProductUpdateInput{}
	userCreateRpcIn := ProductUpdateRpcIn{}
